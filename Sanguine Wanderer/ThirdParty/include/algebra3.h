...
public:
	...
	mat4 inverseJacobian() const; //inverse for jacobian matrices
	mat4 pseudoInverse() const;
...
inline mat4 mat4::inverseJacobian() const // Gauss-Jordan elimination with partial pivoting
{
    mat4 a(*this), // As a evolves from original mat into identity
            b(identity3D()); // b evolves from identity into inverse(a)
    int i, j, i1;
...
inline mat4 mat4::pseudoInverse() const
{

	////first convert mat4 to gsl_matrix
	//int m = 3;
	//int n = 4;
	//mat4 thismatrix = *this;
	//gsl_matrix * A = gsl_matrix_alloc(m, n);
	//for(int i = 0; i < m; i++) {
	//	for(int j = 0; j < n; j++) {
	//		gsl_matrix_set(A, i, j, thismatrix[i][j]);
	//	}
	//}
	////allocate gsl_matrices for U,S,V
 ...
 //
	...
	////take transpose/inverse of matrices and convert back to mat4
	//mat4 Smat, Umat, Vmat;


	//for(int i = 0; i < n; i++) {
	//	if(gsl_vector_get(S, i) != 0) {
	//		Smat[i][i] = 1/gsl_vector_get(S, i);
	//	}
	//}

	//for(int i = 0; i < m; i++) {    
	//	for(int j = 0; j < m; j++) {
	//		Umat[j][i] = gsl_matrix_get(U, i, j);
	//	}
	//}
...
//            0.0, 0.0, 1.0, v[VZ]), vec4(0.0, 0.0, 0.0, 1.0));
    return mat4(vec4(1.0, 0.0, 0.0, 0), vec4(0.0, 1.0, 0.0, 0.0), vec4(
            0.0, 0.0, 1.0, 0.0), vec4(v[VX],v[VY], v[VZ], 1.0));
}
